...
GamePlay1::GamePlay1(std::shared_ptr<Context> &context)
    : m_context(context),
      m_score(0),
...
void GamePlay1::Update(const sf::Time& deltaTime)
{
    if (!m_isPaused)
    {
        m_elapsedTime += deltaTime;

        if (m_elapsedTime.asSeconds() > 0.035)
        {
            for (auto &wall : m_walls)
            {
            ...
            if (m_snake.IsOn(m_food))
            {
                ...
                m_scoreText.setString("Score : " + std::to_string(m_score));
                 ...
                 if (m_score >= 2) {  // Change this condition as needed
                    // Generate position for the second food
                    ...
                    m_food2.setTexture(m_context->m_assets->GetTexture(FOOD2)); // Assuming you have a separate texture for the second food
        ...
        // Increase score and update game state
        m_snake.Grow(m_snakeDirection);
